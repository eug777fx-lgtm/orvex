...
export default async function handler(req, res) {
  ...
  try {
    ...
    // limits. Just enqueue: flip the package to 'producing'. The Vercel
    // cron worker (workflow.js action=process_productions) — or the manual
    // trigger — picks it up and runs the full Higgsfield + ElevenLabs +
    // Creatomate flow reliably out-of-band.
    if (agent_type === 'video_producer') {
      if (!package_id) {
        return res.status(400).json({ error: 'package_id is required' })
      }
      const pkgRows = await sql.query(
        'SELECT id FROM post_packages WHERE id=$1 AND brand_id=$2',
        [package_id, brand_id],
      )
      if (!(pkgRows?.rows ?? pkgRows)?.[0]) {
        return res.status(404).json({ error: 'package not found' })
      }
      await sql.query(
        "UPDATE post_packages SET status='producing', updated_at=now() WHERE id=$1",
        [package_id],
      )
      return res.status(200).json({
        success: true,
        status: 'producing',
        message:
          'Video queued for production — will be ready in 3-5 minutes',
      })
    }
